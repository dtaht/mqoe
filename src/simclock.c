...
u64 * create_simclock(char *filepath) {
		...
		struct stat statbuf;
		void *ptr;
		if ((fd = open(filepath, O_RDWR | O_CREAT)) < 0) errExit("%s Could not open\n",filepath);
		if ((fstat(fd, &statbuf)) < 0) errExit("%s could not stat\n",filepath);
		if ((ptr = mmap(NULL,sizeof(u64),PROT_READ,MAP_SHARED,fd,0)) == MAP_FAILED) errExit1("CSV file Mapping Failed\n");
    close(fd);
    return (u64 *) ptr;
}
...
u64 * open_simclock(char *filepath) {
		...
		struct stat statbuf;
		void *ptr;
		if ((fd = open(filepath, O_READ)) < 0) errExit("%s Could not open\n",filepath);
		if ((fstat(fd, &statbuf)) < 0) errExit("%s could not stat\n",filepath);
		if ((ptr = mmap(NULL,sizeof(u64),PROT_READ,MAP_SHARED,fd,0)) == MAP_FAILED) errExit1("CSV file Mapping Failed\n");
		close(fd);
		return (u64 *) ptr;
...
test_clock() {
		...
		interval.tv_nsec = (long long) ((o->finterval - o->interval.tv_sec) * NSEC_PER_SEC);
